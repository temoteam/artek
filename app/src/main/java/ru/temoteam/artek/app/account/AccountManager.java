...
public class AccountManager {
    ...
    private AlertDialog.Builder generateMsg(byte result) {
        ...
        if (result > 0) {{
            ad.setTitle(activity.getString(ru.temoteam.artek.app.R.string.success));
        }
        } else {
            ad.setTitle(activity.getString(ru.temoteam.artek.app.R.string.failed));
            ad.setMessage(activity.getString(ru.temoteam.artek.app.R.string.not_connected));
        }
        return ad;
    }
    ...
    private void checkToken() {
        ...
        client.newCall(request).enqueue(new okhttp3.Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                Toast.makeText(activity, activity.getString(ru.temoteam.artek.app.R.string.not_connected), Toast.LENGTH_SHORT).show();
                }
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                if (answer.contains("response")) {
                    checkGroup();
                    Toast.makeText(activity, activity.getString(ru.temoteam.artek.app.R.string.success_sign_in), Toast.LENGTH_SHORT).show();
                } else if (answer.contains("error")) {
                    Toast.makeText(activity, activity.getString(ru.temoteam.artek.app.R.string.account_data_not_valid), Toast.LENGTH_SHORT).show();
                ...
                e.printStackTrace();
                                            ...
                                            public void onResponse(Call call, Response response) throws IOException {
                                                ...
                                                if (!answer.contains("1")) {
                                                    ...
                                                    ad.setTitle(activity.getString(ru.temoteam.artek.app.R.string.subscribe_us));  // заголовок
                                                    ad.setMessage(activity.getString(ru.temoteam.artek.app.R.string.subscribe_long)); // сообщение
                                                    ad.setPositiveButton(activity.getString(ru.temoteam.artek.app.R.string.subscribe), new DialogInterface.OnClickListener() {
                                                        public void onClick(DialogInterface dialog, int arg1) {
                                                            subscribeGroup();
                                                        }
                                                    });
                                                    ad.setCancelable(true);
                                                    ad.setNegativeButton(activity.getString(ru.temoteam.artek.app.R.string.later), new DialogInterface.OnClickListener() {
                                                        public void onClick(DialogInterface dialog, int arg1) {
        ...
        client.newCall(request).enqueue(new okhttp3.Callback() {
                ...
                e.printStackTrace();
                                            ...
                                            public void onResponse(Call call, Response response) throws IOException {
                                                String answer = response.body().string();
                                                if (answer.contains("1")) {
                                                    Toast.makeText(activity, activity.getString(ru.temoteam.artek.app.R.string.subscribe_thank), Toast.LENGTH_SHORT).show();
                                                }
